...
app.get('/oauth', (req, res) => {
  ...
  clever.runOAuthFlow(code, (err, user) => {
    ...
    loadUserInformation(user, (err, userInfo) => {
      if (err) { return Error(err, res); }

      req.session.me = userInfo.me;
      req.session.sections = userInfo.sections;
...
function createUser(user) {
  ...
  // You'd probably also want to query the Clever API for the user's name, etc. and store it.
}

// Normally, you'd save the user's name, sections, etc. in a database, and query this information when the user logs in.
// In our case, we don't have a database, so we query the Clever API directly on login, and put information onto the session
function loadUserInformation(user, cb) {
  // Fetch information (name, etc.) about the current user
  clever.getMyInfo(user.id, user.type, user.token, (err, me) => {
    if (err) { return cb(err); }

    // Fetch the sections that the student is in, or that the teacher teaches, along with student information
    clever.getMySectionsWithStudents(user.id, user.type, user.token, (err, sections) => {
      if (err) { return cb(err); }

      cb(undefined, { me, sections });
    });
  });
}
